...
class CTkButton(CTkBaseClass):
    ...
    def right_clicked(self, e):
        if self.rcommand and self.state != 'disabled':
            self.rcommand()
    ...
    def draw(self, no_color_updates=False):
        ...
        if self.image is not None:
            ...
            if self.image_label is None:
                ...
                self.image_label.bind("<Button-1>", self.clicked)
                self.image_label.bind("<Button-3>", self.right_clicked)
    ...
    def configure(self, require_redraw=False, **kwargs):
        ...
        if "rcommand" in kwargs:
            self.rcommand = kwargs.pop("rcommand")
